...
#         self.conv2 = nn.Conv2d(30, 50, 3, 1) # conv1 ->conv2 -add-daca au acelasi nr de canale(/concat)- conv3 (model rezidual ResNet-8a
...
#         x = F.max_pool2d(x, 2, 2)  # primul 2=kernel size=2(matrice 2/2), al 2-lea=stride(se deplaseaza matricea cu 2 poz) 20,13/13
#         # ->20,13,13
...
class Net(nn.Module):
    def __init__(self):  # descriem layerele pe care le avem de folosit
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(1, 30, 3, 1, padding=1)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(
            train_loader):  # batch-se face media si variatia datelor pt a putea fi normalizate
